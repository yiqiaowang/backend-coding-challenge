defmodule Suggestions.TrieTest do
  ...
  describe "lookup in trie" do
    test "with valid key terminating at internal node", context do
      assert Trie.get(
               Trie.insert(Trie.insert(Trie.new(), "a", context[:a]), "ab", context[:ab]),
               "a"
             ) == context[:a]
    end
    ...
    test "with valid key terminating at leaf", context do
      assert Trie.get(
               Trie.insert(Trie.insert(Trie.new(), "a", context[:a]), "aab", context[:ab]),
               "aab"
             ) == context[:ab]
    end
    ...
    test "containing different suffixes", context do
      assert Trie.get(
               Trie.insert(Trie.insert(Trie.new(), "ab", context[:a]), "ac", context[:ab]),
               "ab"
             ) == context[:a]
    ...
    test "containing different prefixes", context do
      ...
      assert Trie.get(
               Trie.insert(Trie.insert(Trie.new(), "xy", context[:a]), "ij", context[:ab]),
               "ij"
             ) == context[:ab]
    end
  end
